...
Problem Statement
Implement a function called MaxMin(arr), which will rearrange the elements of a sorted array so that the first position will have the largest number, second will have the smallest, third will have the second largest, and so on and so forth. */
...
// So at the first iteration, i = 0, therefore, by replacing i with 0 in arr[arr.length - (i+1)] we get arr[arr.length-(0+1)] = arr[arr.length-2]. This is equivalent to the lst element of the array, and as the array is sorted, the largest element in the array is appended to the result array first and then the 0th element. NExt, the second largest and the second smallest are appended and so on until the end.
